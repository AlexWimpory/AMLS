...
from B2.B2_model_labeler import ModelLabelEncoder
from B2.B2_model_structures import *
from B2.B2_model_plotter import plot_history, plot_confusion_matrix
from B2_model_evaluator import calculate_confusion_matrix
from B2 import B2_features_config
from B2_file_utils import load_object
import numpy as np
...
class ImageFeaturesModel:
    ...
    def train_model(self, x_train, y_train, x_val, y_val):
        ...
        history = self.model.fit(x_train, y_train, batch_size=B2_features_config.num_batch_size,
                                 epochs=B2_features_config.num_epochs, validation_data=(x_val, y_val),
...
def train_and_test_model(features, le, model):
    ...
    x_train, x_test, y_train, y_test = train_test_split(features, le.encoded_labels,
                                                        test_size=1 - B2_features_config.train_ratio,
                                                        random_state=44)
    x_cv, x_test, y_cv, y_test = train_test_split(x_test, y_test,
                                                  test_size=B2_features_config.test_ratio / (
                                                          B2_features_config.test_ratio + B2_features_config.validation_ratio),
                                                  random_state=44)
    print(x_train.shape)
...
def trainer():
    features_and_labels = load_object('B2.data')
    ...
    mdl = ImageFeaturesModel('B2', label_encoder, mdl_structure)
    mdl.compile()
